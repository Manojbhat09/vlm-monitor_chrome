// Wrap the entire content script in an IIFE to prevent variable conflicts
(function() {
  // Using unique variable names to avoid conflicts with page scripts
  let _jcrop, _selection;
  let _overlay = ((active) => (state) => {
    active = typeof state === 'boolean' ? state : state === null ? active : !active;
    $('.jcrop-holder')[active ? 'show' : 'hide']();
    chrome.runtime.sendMessage({message: 'active', active});
  })(false);

  // If a variable named 'vars' is used in this script, rename it to _vars
  // (or avoid using it completely)

var image = (done) => {
  // Check for and remove existing image first
  var existingImage = document.getElementById('fake-image');
  if (existingImage) {
    existingImage.remove();
    log('Removed existing fake-image element');
  }
  ...
  img.onerror = () => {
    log('Error loading pixel.png for Jcrop');
...
var init = (done) => {
  $('#fake-image').Jcrop({
    ...
    $('.jcrop-hline, .jcrop-vline').css({
      ...
      if (_selection) {
        _jcrop.setSelect([
          _selection.x, _selection.y,
          _selection.x2, _selection.y2
      ])
    }
...
var capture = (force) => {
  chrome.storage.sync.get((config) => {
      if (_selection && (config.method === 'crop' || (config.method === 'wait' && force))) {
        _jcrop.release()
      setTimeout(() => {
          var selectionCopy = _selection
        chrome.runtime.sendMessage({
          message: 'capture', format: config.format, quality: config.quality
        }, (res) => {
            _overlay(false)
            crop(res.image, selectionCopy, devicePixelRatio, config.scaling, config.format, (image) => {
...
window.addEventListener('resize', ((timeout) => () => {
  ...
  timeout = setTimeout(() => {
    // Check if _jcrop exists before destroying
    if (_jcrop && typeof _jcrop.destroy === 'function') {
      try {
        _jcrop.destroy();
        log('Jcrop instance destroyed on resize.');
        _jcrop = undefined; // Explicitly undefine it after destroy
      } catch (e) {
        log('Error destroying Jcrop:', e);
        _jcrop = undefined; // Ensure it's undefined even if destroy fails
      }
    } else {
      log('Resize event: Jcrop instance not found or already destroyed.');
    }
    
    // Re-initialize - ensure image function handles potential existing #fake-image
    image(() => init(() => _overlay(null)))
